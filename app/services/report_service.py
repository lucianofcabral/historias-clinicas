...
class ReportService:
    ...
    def generate_patient_history_pdf(patient_id: int) -> bytes:
        ...
        patient_data = [
            ...
            ['Fecha de Nacimiento:', patient.birth_date.strftime('%d/%m/%Y') if patient.birth_date else 'N/A'],
        ...
        if consultations:
            ...
            for i, consultation in enumerate(consultations, 1):
                ...
                consult_header = f"Consulta #{i} - {consultation.consultation_date.strftime('%d/%m/%Y')}"
                ...
                if consultation.next_visit:
                    consult_data.append(['Próxima Visita:', consultation.next_visit.strftime('%d/%m/%Y')])
        ...
        if studies:
            ...
            for i, study in enumerate(studies, 1):
                ...
                study_data = [
                    ...
                    ['Fecha:', study.study_date.strftime('%d/%m/%Y') if study.study_date else 'N/A'],
    ...
    def generate_patient_history_excel(patient_id: int) -> bytes:
        ...
        if consultations:
            ...
            # Encabezados
            headers = ['Fecha', 'Motivo', 'Síntomas', 'Diagnóstico', 'Tratamiento', 
                      'Presión', 'FC', 'Temp', 'Peso', 'Altura', 'Notas', 'Próxima Visita']
            ...
            for row, consultation in enumerate(consultations, 2):
                ws_consult.cell(row, 1, consultation.consultation_date.strftime('%d/%m/%Y') if consultation.consultation_date else '')
                ...
                ws_consult.cell(row, 12, consultation.next_visit.strftime('%d/%m/%Y') if consultation.next_visit else '')
        ...
        if studies:
            ...
            # Encabezados
            headers = ['Fecha', 'Tipo', 'Nombre', 'Institución', 'Médico', 
                      'Resultados', 'Observaciones', 'Diagnóstico', 'Estado']
            ...
            for row, study in enumerate(studies, 2):
                ...
                ws_studies.cell(row, 1, study.study_date.strftime('%d/%m/%Y') if study.study_date else '')
    ...
    def generate_consultations_report_pdf(
        ...
        with Session(engine) as session:
            ...
            patients = {p.id: p for p in session.exec(select(Patient).where(Patient.id.in_(patient_ids))).all()}
        ...
        if consultations:
            ...
            for consultation in consultations:
                ...
                data.append([
                    ...
                    patient_name,
                    consultation.reason[:50] + '...' if consultation.reason and len(consultation.reason) > 50 else consultation.reason or '',
                    consultation.diagnosis[:50] + '...' if consultation.diagnosis and len(consultation.diagnosis) > 50 else consultation.diagnosis or '',
                ])
    ...
    def generate_studies_report_excel(
        ...
        # Encabezados
        headers = ['Fecha', 'Paciente', 'Tipo', 'Nombre', 'Institución',
                  'Médico', 'Resultados', 'Diagnóstico', 'Estado']
